...
import { createProduct, updateProduct } from '../hooks/useFetch2.js';
import uuid from 'react-uuid';
// import { comparteContexto } from '../contexts/Context.jsx';
import { comparteContexto } from "../contexts/Context.jsx"
const Modal = ({ tareaSeleccionada, guardarTarea, closeModal }) => {
    ...
    }, [tareaSeleccionada, setNombre1, setDificultad1, setDescripcion1]);

    const handleSubmit = (e) => {
        e.preventDefault();

        if (!nombre1.trim() || !dificultad1.trim()) {
        ...
        createProduct(nuevaTarea); // Llamar a la función para crear producto (implementar en useFetch2.js)
        guardarTarea(nuevaTarea);
    ...
    const handleEditSubmit = (e) => {
        ...
        const idProducto = tareaSeleccionada.id;
        ...
        updateProduct(idProducto, productoEditado); // Llamar a la función para actualizar producto (implementar en useFetch2.js)
        ...
        <div className="py-6">
            <div className="py-40">
                ...
                <div className="fixed left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 py-10">
                    <div className="max-h-full py-10 w-full max-w-xl overflow-y-auto sm:rounded-2xl bg-white my-8 p-6">
                        <div className="w-full">
                            <form className="w-full px-5" onSubmit={modalEdit ? handleEditSubmit : handleSubmit}>
                                <button
                                    className="p-3 bg-sky-500 hover:bg-sky-600 rounded-full text-white w-full font-semibold"
                                    onClick={closeModal}
                                >
                                    Close
                                </button>

                                <label className="block mb-2 text-lg font-bold text-gray-800">Product:</label>
                                <select
                                    className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-sky-200"
                                    value={dificultad1}
                                    onChange={(e) => setDificultad1(e.target.value)}
                                >
                                    ...
                                    <option value="3">Tesla Tires</option>
                                    <option value="4">Intel Core i7</option>
                                </select>

                                <label className="block mb-2 text-lg font-bold text-gray-800">Price:</label>
                                <input
                                    className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-sky-200"
                                    type="number"
                                    value={nombre1}
                                ...
                                <label className="block mb-2 text-lg font-bold text-gray-800">Name:</label>
                                <textarea
                                    value={descripcion1}
                                    onChange={(e) => setDescripcion1(e.target.value)}
                                    className="block w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-5000 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500 h-32"
                                    placeholder="Enter description..."
                                ></textarea>

                                <div className="m-8 my-20 max-w-[400px] mx-auto">
                                    <div className="space-y-4">
                                        <button className="p-3 bg-black hover:bg-slate-900 rounded-full text-white w-full font-semibold" type="submit">
                                            {modalEdit ? 'Edit Product' : 'Add Product'}
                                        </button>
                                    </div>
                                </div>
                            </form>
